...
Kimi K2.5 supports **256K tokens** of context with **64K max output**. Claude Code supports up to **1M tokens** of context (with automatic context compaction to keep conversations going indefinitely) but caps at **32K max output** per response.

Claude actually wins on context window by a wide margin — **1M vs 256K**. But Kimi's **64K output limit** is double Claude's 32K, which matters when generating large files or comprehensive documentation in a single response.
